...
def parse_args():
    parser = argparse.ArgumentParser(description='Gulf of Mexico Series Interpolation')
...
def train_ot_cfm(args, X, n_times, device):
    ...
    # Save model
    torch.save(ot_cfm_model.state_dict(), f"../models/GoM/ot_cfm_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/GoM/ot_cfm_traj_seed_{args.seed}.npz", trajectory=traj_ot_cfm.numpy())
...
def train_sf2m(args, X, n_times, device):
    ...
    # Save model
    torch.save(sf2m_model.state_dict(), f"../models/GoM/sf2m_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/GoM/sb-cfm_traj_seed_{args.seed}.npz", trajectory=traj_sb_cfm.numpy())
    ...
    # Save trajectory
    np.savez(f"../results/GoM/sf2m_traj_seed_{args.seed}.npz", trajectory=sde_traj.numpy())
...
def main():
    ...
    os.makedirs("../models/GoM", exist_ok=True)
    os.makedirs("../results/GoM", exist_ok=True)
    ...
    print(f"Models and trajectories for GoM saved with seed {args.seed}")
