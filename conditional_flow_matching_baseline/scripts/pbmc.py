#!/usr/bin/env python
# PBMC Series Interpolation Script
...
# Try next lines because can't get pbmc to work on script (but works on notebook)
def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
...
def parse_args():
    parser = argparse.ArgumentParser(description='PBMC Series Interpolation')
...
def get_data():
    data = np.load(f"../../Supercloud/realdata/data/processed_pbmc_data_sub500_every_2_until20.npz")
    data_val = np.load(f"../../Supercloud/realdata/data/processed_pbmc_data_sub500_every_2_until20_interp_val.npz")
...
def train_ot_cfm(args, X, n_times, device):
    ...
    # Save model
    torch.save(ot_cfm_model.state_dict(), f"../models/pbmc/ot_cfm_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/pbmc/ot_cfm_traj_seed_{args.seed}.npz", trajectory=traj_ot_cfm.numpy())
...
def train_sf2m(args, X, n_times, device):
    ...
    # Save model
    torch.save(sf2m_model.state_dict(), f"../models/pbmc/sf2m_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/pbmc/sb-cfm_traj_seed_{args.seed}.npz", trajectory=traj_sb_cfm.numpy())
    ...
    device=x0.device,
    dtype=x0.dtype,
    entropy=int(args.seed),  # Explicit entropy
    ...
    # Save trajectory
    np.savez(f"../results/pbmc/sf2m_traj_seed_{args.seed}.npz", trajectory=sde_traj.numpy())
...
def main():
    ...
    os.makedirs("../models/pbmc", exist_ok=True)
    os.makedirs("../results/pbmc", exist_ok=True)
    ...
    print(f"Models and trajectories for `PBMC` saved with seed {args.seed}")
