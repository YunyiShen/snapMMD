...
def get_data():
    data = np.load(f"../../Supercloud/missingobs/data/Repressilator_data.npz")
    data_val = np.load(f"../../Supercloud/missingobs/data/Repressilator_data_interp_val.npz")   
...
def train_ot_cfm(args, X, n_times, device):
    ...
    # Save model
    torch.save(ot_cfm_model.state_dict(), f"../models/repressilator_missingobs/ot_cfm_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/repressilator_missingobs/ot_cfm_traj_seed_{args.seed}.npz", trajectory=traj_ot_cfm.numpy())
...
def train_sf2m(args, X, n_times, device):
    ...
    # Save model
    torch.save(sf2m_model.state_dict(), f"../models/repressilator_missingobs/sf2m_seed_{args.seed}.pt")
    ...
    # Save trajectory
    np.savez(f"../results/repressilator_missingobs/sb-cfm_traj_seed_{args.seed}.npz", trajectory=traj_sb_cfm.numpy())
    ...
    # Save trajectory
    np.savez(f"../results/repressilator_missingobs/sf2m_traj_seed_{args.seed}.npz", trajectory=sde_traj.numpy())
...
def main():
    ...
    os.makedirs("../models/repressilator_missingobs", exist_ok=True)
    os.makedirs("../results/repressilator_missingobs", exist_ok=True)
    ...
    print(f"Models and trajectories for `Repressilator` missing obs saved with seed {args.seed}")
