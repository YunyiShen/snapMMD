...
def get_metric(kind, task_name, seed = 42):
    ...
    if os.path.exists(f"./interpolation/{kind}_{task_name}_interpolation_{seed}.npz"):
        ...
        interpolation = np.load(f"./interpolation/{kind}_{task_name}_interpolation_{seed}.npz")['interpolation'][-1]
        #breakpoint()
        if kind == "missingobs":
            #breakpoint()
            interpolation = interpolation[:, :interpolation.shape[-1]//2]
        #breakpoint()
        return np.array([myMMD(torch.tensor(X_val[i]), 
                      torch.clamp(torch.tensor(interpolation[i]), -1e8, 1e8)).cpu().numpy().item()
